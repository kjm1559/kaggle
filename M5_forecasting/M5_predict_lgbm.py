...
def rmse(true, pred):
    ...
    loss_1 = K.sqrt(K.mean(K.square(true[:, 0:1] - (pred * 800)) + 1e-18))
    return loss_1

def rmsse(true, pred):
    assert pred.shape[0]==true.shape[0]
    # min : 0.03571428571428571
    loss_1 = K.mean(K.sqrt(K.square(true[:, 0:1] - (pred * 800)) / K.clip(true[:, 1:2], 1, K.max(true[:, 1:2])) + 1e-18))
    return loss_1    

def wrmsse(true, pred):
    assert pred.shape[0]==true.shape[0]
    sqrt_data = K.sqrt(K.square(true[:, 0:1] - (pred * 800)) / K.clip(true[:, 1:2], 1, K.max(true[:, 1:2])) + 1e-18) # 같은 제품일 경우 고려, 기간이 같아야함
...
def rmsse_lgbm(true, pred):
    # assert pred.shape[0]==true.shape[0]
    # min : 0.03571428571428571
    predict = np.array(pred.label)
    print(predict.shape, true.shape, true[0], true)
    # loss_1 = K.mean(K.sqrt(K.square(true[:, 0:1] - (predict)) / K.clip(true[:, 1:2], 1, K.max(true[:, 1:2])) + 1e-18))
    loss_1 = np.mean(np.sqrt(np.square(true[:, 0] - predict) / np.clip(true[:, 1], 1, np.max(true[:, 1])) + 1e-18))
    return 'rmsse', loss_1, True
...
def test_lgb_total():
    ...
    df["sales_lgb_2"] = m_lgb.predict(df[train_columns])
    df.to_pickle('train_data_df_lgb.pkl')
...
def test_lgb():
    ...
    for tdelta in range(0, 28 + 28):
        ...
        create_lag_features_for_test(tst, day)  # faster  
        # tst = tst.loc[tst.date == day, train_columns]
        te.loc[((te.date == day), "sales")] = m_lgb.predict(tst[(tst.date == day)][train_columns])
